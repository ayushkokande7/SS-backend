...
const signin = async (req, res) => {
  try {
    ...
    const [rows] = await DB.query(
      "SELECT id, fname, lname, email,verified,password FROM users WHERE email = ?",
      [email]
    );
    if (rows.length == 0) {
    ...
    console.log(error);
    res.Response(500, "Something went wrong", error);
  }
};
...
const signup = async (req, res) => {
  try {
    ...
    if (rows[0].count == 0) {
      ...
      const [rows] = await DB.query(
        `INSERT INTO users (fname, lname, email, phone, password,otp) VALUES (?, ?, ?, ?, ?,?)`,
        [fname, lname, email, phone, hashedPassword, OTP]
      );
      if (rows.affectedRows == 0) {
    ...
    return res.Response(500, error, null);
  }
};
